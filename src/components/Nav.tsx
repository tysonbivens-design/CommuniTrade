...
export default function Nav({ page, setPage, user, profile, notifCount, onSignIn, onSignUp, onSignOut, onLocationChange }: NavProps) {
  ...
  const LocationToggle = () => user ? (
    <div style={{ display: 'flex', gap: '0.4rem', marginBottom: '0.75rem' }}>
      ...
      >
        {fetchingGPS ? 'Locating...' : 'Current Location'}
      </button>
    </div>
  ) : null
    ...
    <>
            ...
            {pillOpen && user && (
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  <span style={{ fontSize: '0.88rem', color: 'var(--bark)' }}>{displayLabel}</span>
      ...
      <div className={`${styles.drawer} ${drawerOpen ? styles.drawerOpen : ''}`}>
        <div className={styles.drawerHeader}>
          ...
          <button className={styles.drawerClose} onClick={() => setDrawerOpen(false)}>X</button>
        </div>
